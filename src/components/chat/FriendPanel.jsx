 ...
 * POST /api/users/friend/:id/accept
 * POST /api/users/friend/:id/decline
 */
...
export default function FriendPanel({ onClose, onCount }) {
  ...
  async function accept(id) {
    ...
    setReqs(p => p.filter(r => r.from?._id !== id))
    loadFriends()
  }
            ...
            : friends.map(f => (
                <div key={f._id} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '8px 14px', borderBottom: '1px solid #f9fafb' }}>
                  <div style={{ position: 'relative', flexShrink: 0 }}>
                    <img src={f.avatar || '/default_images/avatar/default_guest.png'} alt="" style={{ width: 34, height: 34, borderRadius: '50%', objectFit: 'cover' }} onError={e => (e.target.src = '/default_images/avatar/default_guest.png')} />
