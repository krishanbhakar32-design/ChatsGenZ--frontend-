...
export default function DMPanel({ me, socket, onClose, onCountChange }) {
  ...
  const [active,   setActive]  = useState(null) // {userId, username, avatar, rank}
    ...
    <div style={{position:'absolute',right:0,top:'calc(100% + 6px)',background:'#fff',border:'1px solid #e4e6ea',borderRadius:14,width:340,height:480,display:'flex',flexDirection:'column',boxShadow:'0 8px 28px rgba(0,0,0,.14)',zIndex:999,overflow:'hidden'}}>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'11px 14px',borderBottom:'1px solid #f3f4f6',flexShrink:0}}>
        {active ? (
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            ...
            <img src={active.avatar||'/default_images/avatar/default_guest.png'} alt="" style={{width:26,height:26,borderRadius:'50%',objectFit:'cover',border:`1.5px solid ${GBR(active.gender,active.rank)}`}} onError={e=>{e.target.src='/default_images/avatar/default_guest.png'}}/>
      ...
      {!active && (
        <div style={{flex:1,overflowY:'auto'}}>
          ...
          {!load&&convos.length===0&&<div style={{textAlign:'center',padding:'32px 16px',color:'#9ca3af'}}><i className="fi fi-sr-envelope" style={{fontSize:32,display:'block',marginBottom:8,opacity:0.3}}/><p style={{fontSize:'0.875rem',fontWeight:600}}>No messages yet</p></div>}
          {convos.map(c=>(
            ...
            >
              <div style={{position:'relative',flexShrink:0}}>
                <img src={c.avatar||'/default_images/avatar/default_guest.png'} alt="" style={{width:38,height:38,borderRadius:'50%',objectFit:'cover',border:`1.5px solid ${GBR(c.gender,c.rank)}`}} onError={e=>{e.target.src='/default_images/avatar/default_guest.png'}}/>
      ...
      {active && (
        <>
          <div style={{flex:1,overflowY:'auto',padding:'8px 0'}}>
            {messages.map((m,i)=>{
              const isMine = m.from===me?._id||m.from===me?.username
              return (
