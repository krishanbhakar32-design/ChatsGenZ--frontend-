 ...
 * GiftPanel.jsx — FIXED
 * Fixes:
 * 1. /default_images/icons/gold.svg → /icons/gold.svg
 * 2. Gift icons from backend now use full URL or /gift/ folder correctly
 * 3. Added REST API fallback for sending gifts (socket + API)
 * 4. Added proper error handling and user feedback
 */
import { useState, useEffect } from 'react'
const API = import.meta.env.VITE_API_URL || 'https://chatsgenz-backend-production.up.railway.app'

// Resolve gift icon URL from backend data
function resolveGiftIcon(iconField) {
  if (!iconField) return null
  if (iconField.startsWith('http')) return iconField        // Full URL
  if (iconField.startsWith('/')) return iconField           // Already absolute
  return `/gift/${iconField}`                               // FIX: relative → /gift/
}
...
export default function GiftPanel({ targetUser, myGold, onClose, onSent, socket, roomId }) {
  ...
  async function sendGift() {
    ...
    try {
      // Try socket first (real-time notification in room)
      if (socket && roomId && targetUser._id) {
        socket.emit('sendGift',{ toUserId: targetUser._id||targetUser.userId, giftId: selected._id, roomId })
        setSending(false)
        onSent?.()
        return
      }
  ...
  return (
    <div onClick={onClose} style={{position:'fixed',inset:0,zIndex:1001,background:'rgba(0,0,0,.45)',backdropFilter:'blur(4px)',display:'flex',alignItems:'center',justifyContent:'center',padding:16}}>
      <div onClick={e=>e.stopPropagation()} style={{background:'#fff',borderRadius:18,maxWidth:380,width:'100%',overflow:'hidden',boxShadow:'0 16px 48px rgba(0,0,0,.18)'}}>
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'14px 16px',borderBottom:'1px solid #f3f4f6'}}>
          <div>
            ...
            <div style={{display:'flex',alignItems:'center',gap:4,marginTop:3}}>
              ...
              <img src="/icons/gold.svg" alt="" style={{width:13,height:13}} onError={()=>{}}/>
              <span style={{fontSize:'0.74rem',fontWeight:700,color:'#d97706'}}>Your balance: {myGold||0} Gold</span>
            </div>
          </div>
        ...
        {/* Category tabs */}
        <div style={{display:'flex',gap:4,padding:'8px 14px',overflowX:'auto',flexShrink:0,borderBottom:'1px solid #f3f4f6'}}>
          {cats.map(c=>(
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:8,padding:'12px 14px',maxHeight:220,overflowY:'auto'}}>
          {filtered.map(g=>(
            ...
            >
              {/* FIX: resolve gift icon path properly */}
              ...
              <div style={{display:'flex',alignItems:'center',gap:2,marginTop:2}}>
                <img src="/icons/gold.svg" alt="" style={{width:10,height:10}} onError={()=>{}}/>
        ...
        <div style={{padding:'10px 14px 14px',borderTop:'1px solid #f3f4f6'}}>
          {err && (
            <div style={{background:'#fef2f2',border:'1px solid #fecaca',borderRadius:8,padding:'8px 12px',fontSize:'0.8rem',color:'#dc2626',marginBottom:8}}>{err}</div>
          )}
          {selected && (
          ...
          <button onClick={sendGift} disabled={!selected||!canAfford||sending}
            ...
            {sending?'Sending...':selected?`Send ${selected.name} for ${selected.price} Gold`:'Select a gift'}
          </button>
        </div>
      </div>
    </div>
