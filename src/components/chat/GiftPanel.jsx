  ...
  useEffect(()=>{
    fetch(`${API}/api/gifts`,{headers:{Authorization:`Bearer ${token}`}})
      .then(r=>r.json()).then(d=>{
        setGifts(d.gifts||[])
        const cats=['all',...new Set((d.gifts||[]).map(g=>g.category))]
        setCats(cats)
      }).catch(()=>{})
  },[])
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'14px 16px',borderBottom:'1px solid #f3f4f6'}}>
          <div>
            <div style={{fontFamily:'Outfit,sans-serif',fontWeight:900,fontSize:'0.95rem',color:'#111827'}}>
              ...
              Send a Gift {targetUser&&`to ${targetUser.username}`}
            </div>
        ...
        <div style={{padding:'10px 14px 14px',borderTop:'1px solid #f3f4f6'}}>
          {selected && (
            <div style={{display:'flex',alignItems:'center',gap:8,padding:'8px 12px',background:'#f5f3ff',borderRadius:9,marginBottom:10,border:'1px solid #ede9fe'}}>
              <img src={selected.icon} alt="" style={{width:28,height:28,objectFit:'contain'}} onError={e=>e.target.style.display='none'}/>
              <div style={{flex:1}}>
          ...
          <button onClick={sendGift} disabled={!selected||!canAfford||sending}
            style={{width:'100%',padding:'11px',borderRadius:10,border:'none',background:selected&&canAfford?'linear-gradient(135deg,#7c3aed,#5b21b6)':'#f3f4f6',color:selected&&canAfford?'#fff':'#9ca3af',fontWeight:800,cursor:selected&&canAfford?'pointer':'not-allowed',fontSize:'0.875rem',fontFamily:'Outfit,sans-serif',display:'flex',alignItems:'center',justifyContent:'center',gap:8}}>
            <i className="fi fi-sr-gift"/>{sending?'Sending...':selected?`Send ${selected.name} for ${selected.price} Gold`:'Select a gift'}
          </button>
        </div>
      </div>
    </div>
