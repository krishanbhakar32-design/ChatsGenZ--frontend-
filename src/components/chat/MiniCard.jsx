 ...
 * 2. Ignore button now calls API (was just a placeholder)
 * 3. Ban button now calls API via socket emit
 * 4. Report button now calls API
 * 5. PM button opens DM (passes through onDM callback)
 */
...
export default function MiniCard({ user, myLevel, pos, onClose, onFull, onGift, socket, roomId, onDM }) {
  ...
  async function ignoreUser() {
    setIgnored(true)
    fetch(`${API}/api/users/${user._id || user.userId}/ignore`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}` },
    }).catch(() => {})
    onClose()
  ...
  const actions = [
    ...
    myLevel >= 2 && !ignored && {
      icon: 'fi-sr-user-block', label: 'Ignore', color: '#6b7280',
      onClick: ignoreUser,
    },
    canMod && {
