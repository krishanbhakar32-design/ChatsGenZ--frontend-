/**
 * MiniCard.jsx
 * Small user card popup shown when clicking an avatar in chat.
 * Connects to backend via REST (friend, report) and socket (call, kick, mute, gift).
 */
import { R, RL, GBR, RIcon } from '../../utils/chatHelpers.js'

const API = import.meta.env.VITE_API_URL || 'https://chatsgenz-backend-production.up.railway.app'

export default function MiniCard({ user, myLevel, pos, onClose, onFull, onGift, socket, roomId }) {
  if (!user) return null
  ...
  const y = Math.min(pos.y, window.innerHeight - 340)
  ...
  const actions = [
    ...
    {
      icon: 'fi-sr-gift', label: 'Gift', color: '#7c3aed',
      onClick: () => { onGift(user); onClose() },
    },
    myLevel >= 2 && {
      icon: 'fi-sr-user-add', label: 'Friend', color: '#059669',
      onClick: () => {
        fetch(`${API}/api/users/friend/${user._id || user.userId}`, {
          method: 'POST',
    ...
    canMod && {
      ...
      onClick: () => socket?.emit('muteUser', { targetUserId: user._id || user.userId, roomId, minutes: 5 }),
    },
    canMod && {
