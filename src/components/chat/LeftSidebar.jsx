/**
 * LeftSidebar.jsx — FINAL FIXED
 * avatar: /default_images/avatar/guest.png ✅
 * gold icon: /default_images/icons/gold.svg ✅
 * room icon: /default_images/rooms/default_room.png ✅
 * leaderboard types fixed to match backend (xp/level/gold/gifts)
 * premium panel fetches real VIP plans
 */
...
function RoomListPanel() {
  ...
  return (
    <div style={{ flex:1, overflowY:'auto' }}>
      ...
      {rooms.length === 0 && <div style={{ textAlign:'center', padding:20, color:'#9ca3af', fontSize:'0.8rem' }}>No rooms</div>}
    </div>
  )
}
...
function StorePanel() {
  ...
  function buy(pack) {
    setMsg(`Payment coming soon! ${pack.g} Gold for ${pack.p}`)
  ...
  return (
    <div style={{ padding:10, flex:1, overflowY:'auto' }}>
      ...
      {msg && <div style={{ background:'#fef3c7', border:'1px solid #fde68a', borderRadius:8, padding:'8px 12px', fontSize:'0.78rem', color:'#92400e', marginBottom:10 }}>{msg}</div>}
      {[{g:100,p:'₹10'},{g:500,p:'₹45'},{g:1000,p:'₹80'},{g:5000,p:'₹350'}].map(pack => (
        <button key={pack.g} onClick={() => buy(pack)}
...
function LeaderboardPanel() {
  ...
  const [type, setType] = useState('xp')  // matches backend: xp/level/gold/gifts
        ...
        {load
          ...
          : data.map((u, i) => (
              <div key={u._id||i} style={{ display:'flex', alignItems:'center', gap:8, padding:'7px 10px', borderBottom:'1px solid #f3f4f6' }}>
                ...
                <img src={u.avatar || DEFAULT_AVATAR} alt="" style={{ width:24, height:24, borderRadius:'50%', objectFit:'cover', flexShrink:0 }} onError={e => (e.target.src = DEFAULT_AVATAR)} />
...
function UsernamePanel() {
  ...
  async function change() {
    ...
    const r = await fetch(`${API}/api/users/change-username`, {
      ...
      body: JSON.stringify({ newUsername: val.trim() }),
    })
    const d = await r.json()
    if (r.ok && d.newToken) localStorage.setItem('cgz_token', d.newToken)
    setMsg(r.ok ? '✅ Username changed! Refresh to see update.' : d.error || 'Failed')
    setBusy(false)
  }
  ...
  return (
    <div style={{ padding:14, flex:1 }}>
      ...
      />
      <button onClick={change} disabled={busy||!val.trim()}
        style={{ width:'100%', padding:'10px', borderRadius:8, border:'none', background:busy||!val.trim()?'#f3f4f6':'linear-gradient(135deg,#1a73e8,#1464cc)', color:busy||!val.trim()?'#9ca3af':'#fff', fontWeight:700, cursor:busy||!val.trim()?'not-allowed':'pointer', fontSize:'0.875rem', fontFamily:'Outfit,sans-serif' }}>
...
function PremiumPanel() {
  ...
  useEffect(() => {
    ...
    ]).then(([p, s]) => { setPlans(p.plans || []); setStatus(s) }).catch(() => {}).finally(() => setLoad(false))
  }, [])

  async function buy(plan) {
    setBuying(plan.id); setMsg('')
    const r = await fetch(`${API}/api/vip/purchase/${plan.id}`, { method:'POST', headers:{ Authorization:`Bearer ${token}` } })
    const d = await r.json()
    setMsg(r.ok ? `✅ VIP activated for ${plan.days} days!` : d.error || 'Failed')
    if (r.ok) setStatus({ isPremium:true, daysLeft:plan.days })
    setBuying(null)
  }
    ...
    <div style={{ padding:12, flex:1, overflowY:'auto' }}>
      {status?.isPremium && (
        <div style={{ background:'linear-gradient(135deg,#7c3aed,#5b21b6)', borderRadius:10, padding:12, marginBottom:12, textAlign:'center', color:'#fff' }}>
          ...
          <div style={{ fontSize:'0.75rem', opacity:0.85 }}>{status.daysLeft} days remaining</div>
        </div>
      )}
      {msg && <div style={{ background:msg.startsWith('✅')?'#d1fae5':'#fee2e2', borderRadius:8, padding:'8px 12px', fontSize:'0.8rem', color:msg.startsWith('✅')?'#065f46':'#dc2626', marginBottom:12 }}>{msg}</div>}
      {plans.map(plan => (
        <div key={plan.id} style={{ background:'#f9fafb', border:'1.5px solid #e4e6ea', borderRadius:10, padding:'12px', marginBottom:8 }}>
          ...
          <div style={{ fontSize:'0.75rem', color:'#9ca3af', marginBottom:8 }}>+{plan.gold} Gold bonus • {plan.days} days</div>
          <button onClick={() => buy(plan)} disabled={buying===plan.id}
