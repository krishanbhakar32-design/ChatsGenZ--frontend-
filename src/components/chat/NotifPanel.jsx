/**
 * NotifPanel.jsx — FINAL FIXED
 * Uses /default_images/notification/ for icons ✅
 * clear-all endpoint fixed ✅
 */
...
const NOTIF_ICONS = {
  gift:    '/default_images/notification/gift.svg',
  friend:  '/default_images/notification/friend.svg',
  like:    '/default_images/notification/like.svg',
  badge:   '/default_images/notification/badge.svg',
  call:    '/default_images/notification/call.svg',
  levelup: '/default_images/notification/level.svg',
  wall:    '/default_images/notification/post.svg',
  default: '/default_images/notification/default.svg',
}

const TYPE_EMOJI = { gift:'🎁', friend:'👥', like:'❤️', badge:'🏅', levelup:'⬆️', call:'📞', wall:'📝' }
    ...
    >
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'11px 14px', borderBottom:'1px solid #f3f4f6', flexShrink:0 }}>
        ...
        <div style={{ display:'flex', gap:6, alignItems:'center' }}>
          {unread > 0 && (
            <button onClick={markAll} style={{ background:'none', border:'none', cursor:'pointer', color:'#1a73e8', fontSize:'0.75rem', fontWeight:600 }}>Mark all read</button>
          )}
          {list.length > 0 && (
            <button onClick={clearAll} style={{ background:'none', border:'none', cursor:'pointer', color:'#9ca3af', fontSize:'0.75rem' }}>Clear</button>
          )}
        ...
        {list.map(n => (
          ...
          >
            <div style={{ width:32, height:32, borderRadius:8, background:'#f3f4f6', display:'flex', alignItems:'center', justifyContent:'center', fontSize:16, flexShrink:0 }}>
              <img
                src={NOTIF_ICONS[n.type] || NOTIF_ICONS.default}
                alt=""
                style={{ width:20, height:20, objectFit:'contain' }}
                onError={e => { e.target.style.display='none'; e.target.parentNode.textContent = TYPE_EMOJI[n.type] || '🔔' }}
              />
            </div>
