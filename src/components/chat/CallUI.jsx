...
export default function CallUI({ call, socket, onEnd }) {
  ...
  useEffect(() => {
    ...
    socket.on('webrtcOffer',       async ({ offer }) => {
      if (pcRef.current) {
        ...
        socket.emit('webrtcAnswer', { toUserId: call.fromUser?._id || call.toUser?._id, answer: ans, callId: call.callId })
      }
    })
  ...
  function startTimer() { timerRef.current = setInterval(() => setDuration(p => p + 1), 1000) }
  ...
  function accept()  { socket?.emit('acceptCall',  { toUserId: call.fromUser?._id, callId: call.callId }); setStatus('active'); startTimer() }
  function decline() { socket?.emit('declineCall', { toUserId: call.fromUser?._id, callId: call.callId }); endCall() }
  function endCall() {
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:1003, background:'rgba(0,0,0,.85)', backdropFilter:'blur(8px)', display:'flex', alignItems:'center', justifyContent:'center' }}>
      <div style={{ background:'#111827', borderRadius:20, padding:'24px 28px', maxWidth:340, width:'90%', textAlign:'center', boxShadow:'0 20px 60px rgba(0,0,0,.5)' }}>
        <img src={other?.avatar || DEFAULT_AVATAR} alt="" style={{ width:72, height:72, borderRadius:'50%', objectFit:'cover', border:'3px solid #374151', margin:'0 auto 12px', display:'block' }} onError={e => { e.target.src=DEFAULT_AVATAR }}/>
        ...
        <div style={{ display:'flex', gap:12, justifyContent:'center' }}>
          ...
          {status === 'outgoing' && (
            <CallBtn icon="fi-sr-phone-slash" color="#ef4444" label="Cancel" onClick={endCall}/>
          )}
          {status === 'active' && <>
