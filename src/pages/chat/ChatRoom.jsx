...
function RankIcon({ rank, size='sm' }) {
  const cls = size==='xs'?'rank-icon-xs':size==='lg'?'rank-icon-lg':size==='md'?'rank-icon-md':'rank-icon-sm'
  const ri  = R(rank)
...
function MiniCard({ user, myLevel, pos, onClose, onViewFull, socket, roomId }) {
  ...
  return (
    <div style={{ position:'fixed', zIndex:9999, top:y, left:x, background:'#fff', border:'1px solid #e4e6ea', borderRadius:12, width:220, boxShadow:'0 8px 28px rgba(0,0,0,.14)', overflow:'hidden' }}
    ...
    >
      ...
      <div style={{ display:'flex', alignItems:'flex-end', gap:8, padding:'0 12px', marginTop:-18, marginBottom:8 }}>
        ...
        <div style={{ paddingBottom:2, minWidth:0 }}>
          <div style={{ fontFamily:'Outfit,sans-serif', fontWeight:800, fontSize:'0.875rem', color:'#111827', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
            {user.username}
          </div>
          <div style={{ display:'flex', alignItems:'center', gap:4 }}>
            <RankIcon rank={user.rank} size="xs"/>
...
function MCBtn({icon,label,color,onClick}) {
  ...
  return (
    <button onClick={onClick} onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)}
      style={{display:'flex',alignItems:'center',gap:4,padding:'6px 7px',background:h?'#e8f0fe':'#f9fafb',border:`1px solid ${h?'#1a73e8':'#e4e6ea'}`,borderRadius:7,cursor:'pointer',fontSize:'0.72rem',fontWeight:600,color:color||'#374151',transition:'all .12s'}}>
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:'#fff',borderRadius:18,maxWidth:340,width:'100%',overflow:'hidden',boxShadow:'0 16px 48px rgba(0,0,0,.18)'}}>
        <div style={{height:88,background:`linear-gradient(135deg,${ri.color}44,#e8f0fe)`,position:'relative'}}>
          ...
          {user.countryCode && user.countryCode!=='ZZ' && (
            <img src={`/icons/flags/${user.countryCode.toLowerCase()}.png`} className="flag-img" alt="" style={{position:'absolute',bottom:10,right:12}} onError={e=>e.target.style.display='none'}/>
          )}
        </div>
        ...
        <div style={{padding:'10px 18px 18px',textAlign:'center'}}>
          ...
          <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:5,margin:'4px 0 12px'}}>
            <RankIcon rank={user.rank} size="sm"/>
...
function PBtn({icon,label,color,onClick}) {
  ...
  return (
    <button onClick={onClick} onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)}
      style={{display:'flex',alignItems:'center',gap:4,padding:'7px 10px',background:h?'#f0f7ff':'#f9fafb',border:`1.5px solid ${h?color||'#1a73e8':'#e4e6ea'}`,borderRadius:8,cursor:'pointer',fontSize:'0.78rem',fontWeight:600,color:color||'#374151',transition:'all .12s'}}>
...
// ── MESSAGE ───────────────────────────────────────────────────
function Msg({ msg, onMiniCard, onMention }) {
  ...
  if (isSystem) return (
    <div style={{textAlign:'center',padding:'3px 0'}}>
      <span style={{fontSize:'0.72rem',color:'#9ca3af',background:'#f3f4f6',padding:'2px 12px',borderRadius:20}}>{msg.content}</span>
    </div>
  )
  ...
  return (
    ...
    >
      ...
      {u.countryCode && u.countryCode!=='ZZ' && (
        <img src={`/icons/flags/${u.countryCode.toLowerCase()}.png`} className="flag-img" alt="" style={{width:16,height:11}} onError={e=>e.target.style.display='none'}/>
      )}
    </div>
  )
}
  ...
  return (
    <div style={{width:210,borderLeft:'1px solid #e4e6ea',background:'#fff',display:'flex',flexDirection:'column',flexShrink:0}}>
      <div style={{display:'flex',alignItems:'center',borderBottom:'1px solid #e4e6ea',flexShrink:0}}>
        ...
        <button onClick={onClose} style={{background:'none',border:'none',cursor:'pointer',color:'#9ca3af',padding:'4px 7px',fontSize:13}}>
          <i className="fi fi-sr-cross-small"/>
        </button>
      </div>
      {tab==='search' && (
        <div style={{padding:'7px 8px',borderBottom:'1px solid #f3f4f6',flexShrink:0}}>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search username..."
            style={{width:'100%',padding:'6px 10px',background:'#f9fafb',border:'1.5px solid #e4e6ea',borderRadius:7,fontSize:'0.8rem',outline:'none',boxSizing:'border-box',color:'#111827',marginBottom:6}}
          ...
          <div style={{display:'flex',gap:4}}>
            <select value={genderF} onChange={e=>setGenderF(e.target.value)}
              ...
              <option value="all">All</option><option value="male">Male</option>
              <option value="female">Female</option><option value="couple">Couple</option><option value="other">Other</option>
            </select>
      ...
      <div style={{flex:1,overflowY:'auto'}}>
        {filtered.length===0
          ? <p style={{textAlign:'center',color:'#9ca3af',fontSize:'0.78rem',padding:'16px 10px'}}>
              {tab==='friends'?'No friends':tab==='staff'?'No staff':'No users'}
            </p>
    ...
    <div style={{display:'flex',height:'100%',flexShrink:0}}>
      ...
      {panel && (
        <div style={{width:240,background:'#fff',borderRight:'1px solid #e4e6ea',display:'flex',flexDirection:'column',boxShadow:'2px 0 8px rgba(0,0,0,.06)'}}>
          ...
          {(panel==='wall'||panel==='news') && <ComingSoon label={ITEMS.find(i=>i.id===panel)?.label}/>}
        </div>
      )}
    </div>
  )
    ...
    <div style={{padding:10,flex:1,overflowY:'auto'}}>
      {GAMES.map(g=>(
        ...
        >
          ...
          <div><div style={{fontSize:'0.85rem',fontWeight:700,color:'#111827'}}>{g.label}</div><div style={{fontSize:'0.72rem',color:'#9ca3af'}}>{g.desc}</div></div>
        </button>
      ))}
    </div>
  )
  ...
  return (
    <div style={{padding:10,flex:1,overflowY:'auto'}}>
      <div style={{background:'linear-gradient(135deg,#fbbf24,#f59e0b)',borderRadius:10,padding:12,marginBottom:10,textAlign:'center'}}>
        <img src="/icons/ui/gold.svg" alt="" style={{width:28,height:28,margin:'0 auto 4px',display:'block'}} onError={()=>{}}/>
      ...
      {[{g:100,p:'₹10'},{g:500,p:'₹45'},{g:1000,p:'₹80'},{g:5000,p:'₹350'}].map(p=>(
        ...
        >
          <div style={{display:'flex',alignItems:'center',gap:6}}>
            ...
            <span style={{fontSize:'0.85rem',fontWeight:700,color:'#111827'}}>{p.g} Gold</span>
          </div>
  ...
  return (
    <div style={{padding:14,flex:1}}>
      <div style={{background:'#fef3c7',border:'1px solid #fde68a',borderRadius:9,padding:'10px 12px',marginBottom:12,fontSize:'0.8rem',color:'#92400e',display:'flex',alignItems:'center',gap:6}}>
        <img src="/icons/ui/gold.svg" alt="" style={{width:13,height:13}} onError={()=>{}}/>
...
function PremiumPanel() {
  return (
    <div style={{padding:14,flex:1,textAlign:'center'}}>
      ...
      <p style={{fontSize:'0.83rem',color:'#6b7280',marginBottom:14,lineHeight:1.6}}>Exclusive rank, custom name color, premium badge and more!</p>
...
function ComingSoon({label}) {
  return <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',color:'#9ca3af',fontSize:'0.875rem'}}>{label} coming soon</div>
}

// ── RADIO ─────────────────────────────────────────────────────
function RadioPanel({ onClose }) {
  const [allStations, setAll] = useState([])
  const [category,    setCat] = useState(null) // null = show categories
  ...
  const CATS = [
    ...
    {id:'Bengal',    label:'🎵 Bengali',   langs:['Bengali']},
    {id:'Other',     label:'🌐 More',      langs:['Marathi','Hindi/Sanskrit','Instrumental']},
  ]

  const catStations = category ? allStations.filter(s => {
    const cat = CATS.find(c=>c.id===category)
    return cat?.langs.includes(s.language)
  }) : []

  function play(s) {
    if (current?.id===s.id && playing) { audioRef.current?.pause(); setPlay(false); return }
    setCur(s)
    if (audioRef.current) {
    ...
    <div style={{position:'absolute',bottom:'100%',left:0,right:0,background:'#fff',border:'1px solid #e4e6ea',borderRadius:'10px 10px 0 0',boxShadow:'0 -4px 20px rgba(0,0,0,.12)',maxHeight:'50vh',display:'flex',flexDirection:'column',zIndex:100}}>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'9px 13px',borderBottom:'1px solid #f3f4f6',flexShrink:0}}>
        <div style={{display:'flex',alignItems:'center',gap:7}}>
          ...
          <span style={{fontFamily:'Outfit,sans-serif',fontWeight:800,fontSize:'0.9rem',color:'#111827'}}>
            {playing&&current ? `🎵 ${current.name}` : category ? CATS.find(c=>c.id===category)?.label : 'Radio'}
          </span>
        </div>
        <div style={{display:'flex',gap:6,alignItems:'center'}}>
          {playing && (
            <button onClick={()=>{audioRef.current?.pause();setPlay(false)}}
      ...
      <div style={{flex:1,overflowY:'auto',padding:'6px'}}>
        {!category ? (
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:6,padding:'4px'}}>
            {CATS.map(cat=>{
              const count = allStations.filter(s=>cat.langs.includes(s.language)).length
              return (
                <button key={cat.id} onClick={()=>setCat(cat.id)}
                  ...
                  onMouseLeave={e=>{e.currentTarget.style.background='#f9fafb';e.currentTarget.style.borderColor='#e4e6ea'}}
                >
                  <div style={{fontSize:'1.2rem',marginBottom:4}}>{cat.label.split(' ')[0]}</div>
                  <div style={{fontSize:'0.75rem',fontWeight:700,color:'#374151'}}>{cat.label.split(' ').slice(1).join(' ')}</div>
          ...
          catStations.map(s=>(
            ...
            >
              <div style={{width:30,height:30,background:'#f3f4f6',borderRadius:6,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,fontSize:14}}>
...
function DI({icon,label,color,onClick}) {
  ...
  return (
    <button onClick={onClick} onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)}
      style={{display:'flex',alignItems:'center',gap:8,width:'100%',padding:'8px 10px',background:h?'#f3f4f6':'none',border:'none',cursor:'pointer',color:color||'#374151',fontSize:'0.84rem',fontWeight:600,borderRadius:7,textAlign:'left',transition:'background .12s'}}>
...
function FootBtn({icon,active,onClick,title,badge}) {
  return (
    <button onClick={onClick} title={title}
      style={{position:'relative',background:active?'#e8f0fe':'none',border:'none',cursor:'pointer',color:active?'#1a73e8':'#9ca3af',width:34,height:32,borderRadius:7,display:'flex',alignItems:'center',justifyContent:'center',fontSize:15,transition:'all .15s'}}>
      <i className={`fi ${icon}`}/>
...
export default function ChatRoom() {
  ...
  function connectSocket() {
    ...
    s.on('youAreKicked',  ({reason})=>{toast?.show(reason||'You were kicked','error',5000);setTimeout(()=>nav('/chat'),2000)})
    s.on('accessDenied',  ({msg})=>{toast?.show(msg||'Access denied','error',5000);setTimeout(()=>nav('/chat'),2000)})
    s.on('youAreMuted',   ({minutes,reason})=>toast?.show(`🔇 Muted for ${minutes} minutes${reason?' — '+reason:''}`, 'warn', 6000))
    s.on('levelUp',       ({level,gold})=>toast?.show(`🎉 Level Up! You are now Level ${level}! +${gold} Gold`, 'success', 5000))
    s.on('dailyBonusClaimed',({gold})=>toast?.show(`🎁 Daily bonus: +${gold} Gold!`, 'success', 4000))
    s.on('giftReceived',  ({gift,from})=>toast?.show(`🎁 ${from} sent you a ${gift.name}!`, 'gift', 5000))
    s.on('mentioned',     ({username,roomId:rid})=>toast?.show(`💬 ${username} mentioned you!`, 'info', 4000))
  ...
  // Click username → add to input (no @)
  ...
  return (
    <div style={{height:'100dvh',display:'flex',flexDirection:'column',background:'#fff',overflow:'hidden'}} onClick={()=>setMini(null)}>
      ...
      <div style={{height:48,background:'#fff',borderBottom:'1px solid #e4e6ea',display:'flex',alignItems:'center',padding:'0 8px',gap:6,flexShrink:0,boxShadow:'0 1px 3px rgba(0,0,0,.05)'}}>
        ...
        >
          <img src="/icons/ui/webcam.svg" alt="Cam" style={{width:22,height:22,objectFit:'contain'}}
        ...
        {/* Reports — staff only */}
        {myLevel>=11 && <HBtn icon="fi-sr-flag" title="Reports" badge={notif.reports}/>}
      ...
      <div style={{flex:1,display:'flex',overflow:'hidden',position:'relative'}}>
        ...
        <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
          ...
          <div style={{flex:1,overflowY:'auto',padding:'8px 0'}}>
            {messages.map((m,i)=>(
              <Msg key={m._id||i} msg={m} onMiniCard={handleMiniCard} onMention={handleMention}/>
            ))}
            <div ref={bottomRef}/>
          </div>
          ...
          <div style={{borderTop:'1px solid #e4e6ea',padding:'7px 10px',background:'#fff',flexShrink:0}}>
            <form onSubmit={send} style={{display:'flex',alignItems:'center',gap:6}}>
              ...
              />
              <button type="button" title="Gift" style={{background:'none',border:'none',cursor:'pointer',fontSize:17,padding:'0 2px',flexShrink:0,display:'flex',alignItems:'center'}}>
                <img src="/icons/ui/gift.svg" alt="" style={{width:19,height:19,objectFit:'contain'}} onError={e=>{e.target.outerHTML='<i class="fi fi-sr-gift" style="font-size:17px;color:#9ca3af"></i>'}}/>
              </button>
        ...
        {/* RIGHT SIDEBAR */}
        {showRight && (
          <RightSidebar users={users} myLevel={myLevel} tab={rightTab} setTab={setRightTab}
            onUserClick={u=>{setProf(u);setMini(null)}} onClose={()=>setRight(false)}/>
        )}
      </div>
      ...
      <div style={{position:'relative'}}>
        ...
        <Footer showRadio={showRadio} setShowRadio={setRadio}
