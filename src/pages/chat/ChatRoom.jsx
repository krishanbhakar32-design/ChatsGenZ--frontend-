 ...
 * ChatRoom.jsx — FIXED
 * Fixes:
 * 1. roomPasswordRequired socket event handled (was missing — user got stuck)
 * 2. showDM now renders real DMPanel instead of "coming soon" placeholder
 * 3. All /default_images/avatar/ paths remapped through getAvatarUrl helper
 * 4. /default_images/icons/gift.svg → /icons/gift.svg (correct path)
 * 5. updateStatus socket event missing — now emits on status change via AvatarDropdown
 */
...
// ── ROOM PASSWORD MODAL ───────────────────────────────────────
// FIX: was completely missing — backend sends roomPasswordRequired event
// but nothing handled it, user was stuck
  ...
  return (
    <div onClick={onCancel} style={{ position:'fixed', inset:0, zIndex:1500, background:'rgba(0,0,0,.5)', backdropFilter:'blur(8px)', display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:'#fff', borderRadius:18, padding:'28px 24px', maxWidth:320, width:'100%', boxShadow:'0 24px 64px rgba(0,0,0,.22)', textAlign:'center' }}>
        ...
        <form onSubmit={e=>{ e.preventDefault(); if(!val.trim()){setErr('Enter password'); return} onSubmit(val.trim()) }} style={{ display:'flex', flexDirection:'column', gap:10 }}>
  ...
  return (
    <div onClick={onClose} style={{ position:'fixed', inset:0, zIndex:1000, background:'rgba(0,0,0,.45)', backdropFilter:'blur(4px)', display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:'#fff', borderRadius:18, maxWidth:340, width:'100%', padding:'20px', boxShadow:'0 16px 48px rgba(0,0,0,.18)', textAlign:'center' }}>
        <img src={getAvatarUrl(user.avatar)} alt="" style={{ width:72, height:72, borderRadius:'50%', border:`3px solid ${bdr}`, objectFit:'cover', margin:'0 auto 10px', display:'block' }} onError={e=>(e.target.src='/default_avatar/other.png')}/>
...
export default function ChatRoom() {
  ...
  // FIX: roomPasswordRequired handler state
  const [passRequired, setPassRequired] = useState(null) // { roomId, roomName }
  ...
  function connectSocket() {
    ...
    // FIX: Handle roomPasswordRequired — backend sends this when room has password
    // Previously nothing handled this event, user got stuck with no feedback
    s.on('roomPasswordRequired', ({ roomId: rid, roomName }) => {
      setPassRequired({ roomId: rid, roomName })
    })

    // FIX: Handle userStatusUpdate from other users
    s.on('userStatusUpdated', ({ userId, status: newStatus }) => {
      setUsers(p => p.map(u => u.userId === userId ? { ...u, status: newStatus } : u))
    })

    sockRef.current = s
  }

  // FIX: Submit room password
  ...
  // FIX: Status change — was missing the socket emit in ChatRoom context
  ...
  return (
    <div style={{ height:'100dvh', display:'flex', flexDirection:'column', background:'#fff', overflow:'hidden' }} onClick={closeAll}>
      ...
      <div style={{ height:48, background:'#fff', borderBottom:'1px solid #e4e6ea', display:'flex', alignItems:'center', padding:'0 8px', gap:5, flexShrink:0, boxShadow:'0 1px 3px rgba(0,0,0,.05)' }}>
        ...
        {/* DM — FIX: now renders real DMPanel */}
        ...
        {/* FIX: pass handleStatusChange so status is emitted to socket */}
        ...
        <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
          ...
          <div style={{ borderTop:'1px solid #e4e6ea', padding:'7px 10px', background:'#fff', flexShrink:0, position:'relative' }}>
            ...
            <form onSubmit={send} style={{ display:'flex', alignItems:'center', gap:6 }}>
              ...
              {/* FIX: /default_images/icons/gift.svg → /icons/gift.svg */}
              <button type="button" title="Gift" onClick={e=>{e.stopPropagation();setGiftTgt({_id:null,username:'Room'})}}
                ...
                <img src="/icons/gift.svg" alt="" style={{ width:19, height:19, objectFit:'contain' }} onError={e=>{e.target.outerHTML='<i class="fi fi-sr-gift" style="font-size:17px;color:#9ca3af"></i>'}}/>
              </button>
      ...
      {/* FIX: Room Password Modal — was completely missing */}
      {passRequired&&(
        <RoomPassModal
