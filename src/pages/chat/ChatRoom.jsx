// ============================================================
// ChatRoom.jsx — Main chat room page  [FIXED v2]
// Fixes:
//   - Header & Footer are sticky (flexShrink:0 + zIndex), never scroll
//   - No horizontal scroll on chat or quote bar (overflow:hidden, maxWidth)
//   - tObj passed to <Msg> so theme text colors apply correctly
//   - Footer receives tObj for theme-aware colors
// ============================================================
...
export default function ChatRoom() {
    ...
    s.on('echoMessage',      (payload) => {
      ...
      if (from) {
        setMsgs(p => [...p, {
          ...
          type:'whisper', isEcho:true, content, from, to, sender:from,
  ...
  const handleIgnore    = useCallback((uid) => {
    ...
    setMsgs(prev => prev.map(m => (m.sender?._id===uid || m.sender?.userId===uid) ? {...m, _ignored:true} : m))
  }, [])
  ...
  if (!loading && roomErr) return (
    <div style={{minHeight:'100dvh',background:'#f8f9fa',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:12,padding:16}}>
      ...
      <p style={{color:'#374151',fontWeight:600,textAlign:'center'}}>{roomErr}</p>
  ...
  return (
    ...
    <div style={{height:'100dvh',display:'flex',flexDirection:'column',background:thBg,overflow:'hidden',position:'relative',maxWidth:'100vw',paddingBottom:50}} onClick={closeAll}>
      ...
      {thBgImg && (
        <div style={{position:'fixed',inset:0,backgroundImage:`url(${thBgImg})`,backgroundSize:'cover',backgroundPosition:'center',backgroundRepeat:'no-repeat',zIndex:0,pointerEvents:'none',opacity:0.6}}/>
      )}

      {/* ══════════════ HEADER — sticky, never scrolls ══════════════ */}
      <div style={{
        height:50, flexShrink:0, zIndex:100, position:'relative',
        background:thHeader, borderBottom:`1px solid ${thBorder}22`,
      ...
      {/* ══════════════ BODY — fills space between header & footer ══════════════ */}
      <div style={{flex:1, display:'flex', overflow:'hidden', minHeight:0}}>
        ...
        {/* ── MESSAGES COLUMN ── */}
        <div style={{flex:1, display:'flex', flexDirection:'column', overflow:'hidden', minWidth:0, maxWidth:'100%', background:thBg}}>
          ...
          {/* Scrollable messages — overflowX:hidden prevents horizontal scroll */}
          ...
          {/* ── INPUT BAR ── */}
          <div style={{borderTop:`1px solid ${thBorder}44`,padding:'5px 8px',background:thHeader,flexShrink:0,position:'relative',zIndex:10}}>

            {/* Quote preview — NO horizontal scroll, ellipsis only */}
            {quotedMsg && (
              <div style={{display:'flex',alignItems:'center',gap:8,background:'rgba(99,102,241,0.10)',border:'1px solid #6366f133',borderRadius:8,padding:'5px 10px',marginBottom:5,borderLeft:'3px solid #6366f1',overflow:'hidden',maxWidth:'100%'}}>
                <i className="fa-solid fa-reply-all" style={{fontSize:12,color:'#6366f1',flexShrink:0}}/>
                <div style={{flex:1,minWidth:0,overflow:'hidden'}}>
                  <div style={{fontSize:'0.68rem',fontWeight:800,color:'#6366f1',fontFamily:'Nunito,sans-serif',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>
                    Replying to {quotedMsg.sender?.username || 'Unknown'}
                  </div>
                  <div style={{fontSize:'0.72rem',color:thText+'99',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',fontFamily:'Nunito,sans-serif'}}>
            ...
            {showPlus && (
              <div onClick={e => e.stopPropagation()}
                style={{position:'absolute',bottom:'calc(100% + 5px)',left:6,background:'#fff',border:'1px solid #e4e6ea',borderRadius:12,padding:8,display:'flex',gap:7,boxShadow:'0 4px 20px rgba(0,0,0,.14)',zIndex:50}}>
                {[
            ...
            {/* Input row — maxWidth+overflow prevent horizontal expansion */}
            <form onSubmit={send} style={{display:'flex',alignItems:'center',gap:4,maxWidth:'100%',overflow:'hidden'}}>
              <button type="button" onClick={e => { e.stopPropagation(); setShowPlus(p => !p); setShowEmoji(false); setShowGif(false); setShowYT(false) }}
                style={{width:32,height:32,borderRadius:'50%',border:`1.5px solid ${thBorder}55`,background:showPlus?thLog||'#f3f4f6':'transparent',color:thText,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,flexShrink:0,fontWeight:700,lineHeight:1,transition:'all .15s'}}>
                +
              </button>
              <button type="button" onClick={e => { e.stopPropagation(); setShowEmoji(p => !p); setShowPlus(false) }}
                style={{background:'none',border:'none',cursor:'pointer',color:thText,fontSize:20,padding:'0 1px',flexShrink:0,display:'flex',alignItems:'center',lineHeight:1,opacity:showEmoji?1:0.7}}>
              ...
              <input ref={inputRef} value={input} onChange={handleTyping}
                ...
                disabled={!connected}
                style={{flex:1,minWidth:0,padding:'8px 12px',background:thLog||thBg,border:`1.5px solid ${thBorder}55`,borderRadius:22,color:thText,fontSize:'0.88rem',outline:'none',transition:'border-color .15s',fontFamily:'Nunito,sans-serif'}}
                onFocus={e => e.target.style.borderColor = thAccent}
                onBlur={e  => e.target.style.borderColor = `${thBorder}55`}/>
        ...
        {showRight && (
          <RightSidebar users={users} myId={me?._id} myLevel={myLevel} tObj={tObj}
            onUserClick={(u, e) => {
              if (u._id===me?._id || u.userId===me?._id) { setProf(u) }
              else { handleMiniCard(u, {x:Math.min((e?.clientX||200),window.innerWidth-220), y:Math.min((e?.clientY||100),window.innerHeight-300)}) }
            }}
            onWhisper={u => setWhisper(u)} onClose={() => setRight(false)}/>
        )}
      </div>
      ...
      {miniCardData?.user && (
        <MiniCard user={miniCardData.user} myId={me?._id} myLevel={myLevel} pos={miniCardData.pos}
      ...
      {giftTarget && (
        <GiftPanel targetUser={giftTarget} myGold={me?.gold||0} onClose={() => setGiftTgt(null)} onSent={() => setGiftTgt(null)}
          socket={sockRef.current} roomId={roomId} onGoldSpent={price => setMe(p => p ? {...p, gold:Math.max(0,(p.gold||0)-price)} : p)}/>
      )}
