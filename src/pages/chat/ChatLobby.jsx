...
const RANK_INFO = {
  guest:      { color:'#888888', icon:'guest.svg'      },
  user:       { color:'#aaaaaa', icon:'user.svg'       },
  vipfemale:  { color:'#FF4488', icon:'vipfemale.svg'  },
  vipmale:    { color:'#4488FF', icon:'vipmale.svg'    },
  butterfly:  { color:'#FF66AA', icon:'butterfly.svg'  },
  ninja:      { color:'#777777', icon:'ninja.svg'      },
  fairy:      { color:'#FF88CC', icon:'fairy.svg'      },
  legend:     { color:'#FF8800', icon:'legend.png'     },
  bot:        { color:'#00cc88', icon:'bot.svg'        },
  premium:    { color:'#aa44ff', icon:'premium.svg'    },
  moderator:  { color:'#00AAFF', icon:'moderator.svg'  },
  admin:      { color:'#FF4444', icon:'admin.svg'      },
  superadmin: { color:'#FF00FF', icon:'superadmin.svg' },
  owner:      { color:'#FFD700', icon:'owner.svg'      },
}
const ri = r => RANK_INFO[r] || RANK_INFO.guest
...
function LobbyHeader({ user, onLogout }) {
  ...
  const info      = user?.rankInfo || ri(rank)
  const rankColor = info.color || '#aaa'
  const rankIcon  = info.icon  || 'guest.svg'
  ...
  return (
    <header style={{
      ...
      padding:'0 14px', gap:10, boxShadow:'0 2px 10px rgba(0,0,0,.4)',
    }}>
      {/* Logo — no link, stays in lobby */}
      <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
        <img src="/favicon/favicon-192.png" alt=""
      ...
      <div ref={ref} style={{ position:'relative' }}>
        <button onClick={() => setOpen(o => !o)} style={{
          ...
          padding:'4px 7px', borderRadius:9, transition:'background .15s',
        }}
        ...
        >
          ...
          <div style={{ position:'relative', flexShrink:0 }}>
            <img
              ...
              alt=""
              style={{ width:32, height:32, borderRadius:'50%', objectFit:'cover', border:`2.5px solid ${rankColor}`, display:'block' }}
          ...
          {/* Name + rank icon only (no label) */}
          <div style={{ textAlign:'left', lineHeight:1 }} className="hdr-text">
            <div style={{ fontSize:'0.82rem', fontWeight:700, color:'#fff', maxWidth:90, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
              {user?.username || 'Guest'}
            </div>
            <div style={{ display:'flex', alignItems:'center', gap:4, marginTop:3 }}>
              <img
                src={`/icons/ranks/${rankIcon}`}
                alt=""
          ...
          <i className={`fi fi-sr-angle-${open?'up':'down'}`} style={{ fontSize:9, color:'rgba(255,255,255,.35)' }} />
        </button>
        ...
        {open && (
          <div style={{
            ...
            borderRadius:13, padding:5, minWidth:195,
            ...
            <div style={{ padding:'12px 13px', borderBottom:'1px solid #1e2d3d', marginBottom:4 }}>
              <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8 }}>
                <img
                  src={user?.avatar || '/default_images/avatar/default_guest.png'}
                  style={{ width:40, height:40, borderRadius:'50%', border:`2.5px solid ${rankColor}`, objectFit:'cover', flexShrink:0 }}
                  onError={e => { e.target.src='/default_images/avatar/default_guest.png' }}
                />
                <div style={{ minWidth:0 }}>
                  <div style={{ fontSize:'0.875rem', fontWeight:800, color:'#fff', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                    {user?.username}
                  </div>
                  {/* Rank icon only — no text label */}
                  <img
                    src={`/icons/ranks/${rankIcon}`}
                    alt=""
                    style={{ width:18, height:18, objectFit:'contain', marginTop:3, display:'block' }}
                    onError={e => e.target.style.display='none'}
                  />
                </div>
              </div>

              {/* Gold + Level with icons */}
              {!user?.isGuest && (
                <div style={{ display:'flex', gap:10 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:4 }}>
                    <img src="/icons/ui/gold.svg" alt="" style={{ width:14, height:14 }} onError={e=>e.target.style.display='none'} />
                    <span style={{ fontSize:'0.72rem', fontWeight:700, color:'#fbbf24' }}>{user?.gold || 0}</span>
                  </div>
                  <div style={{ display:'flex', alignItems:'center', gap:4 }}>
                    <img src="/icons/ui/level.svg" alt="" style={{ width:14, height:14 }} onError={e=>e.target.style.display='none'} />
                    <span style={{ fontSize:'0.72rem', fontWeight:700, color:'#60a5fa' }}>{user?.level || 1}</span>
                  </div>
                </div>
              )}
            </div>

            {/* Menu items — icons only with label */}
            {!user?.isGuest && <DBtn icon="fi-sr-user-circle" label="My Profile" onClick={()=>setOpen(false)} />}
            {isStaff && <DBtn icon="fi-sr-settings" label="Admin Panel" color="#f87171" onClick={()=>{ setOpen(false); window.location.href='/admin' }} />}
            <div style={{ borderTop:'1px solid #1e2d3d', marginTop:4, paddingTop:4 }}>
              <DBtn icon="fi-sr-sign-out" label="Logout" color="#f87171" onClick={()=>{ setOpen(false); onLogout() }} />
            </div>
          </div>
        )}
      </div>
...
function RoomRow({ room, onClick }) {
  ...
  const typeIcon = {
    public:  { cls:'fi-sr-globe',      color:'#60a5fa' },
    private: { cls:'fi-sr-lock',       color:'#f87171' },
    premium: { cls:'fi-sr-crown',      color:'#c084fc' },
    staff:   { cls:'fi-sr-shield',     color:'#fbbf24' },
    admin:   { cls:'fi-sr-dashboard',  color:'#f87171' },
    member:  { cls:'fi-sr-user-check', color:'#4ade80' },
  }[room.type] || { cls:'fi-sr-globe', color:'#60a5fa' }
    ...
    >
      ...
      <div style={{
        ...
        <img
          ...
          onError={e => { e.target.style.display='none'; e.target.parentNode.innerHTML='<i class="fi fi-sr-comment-alt" style="color:#374151;font-size:20px"></i>' }}
        />
      </div>
      ...
      <div style={{ flex:1, minWidth:0 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:3 }}>
          {room.isPinned && <span style={{ fontSize:11, flexShrink:0 }}>📌</span>}
          <span style={{ fontFamily:'Outfit,sans-serif', fontWeight:800, fontSize:'0.9rem', color:'#fff', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', flex:1 }}>
            {room.name}
          </span>
          {/* Type icon only */}
          <i className={`fi ${typeIcon.cls}`} style={{ fontSize:13, color:typeIcon.color, flexShrink:0 }} />
          {room.password && <i className="fi fi-sr-lock" style={{ fontSize:11, color:'#6b7280', flexShrink:0 }} />}
        </div>

        {/* Description */}
        {room.description && (
          <div style={{ fontSize:'0.75rem', color:'rgba(255,255,255,.35)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', marginBottom:5 }}>
            {room.description}
          </div>
        )}

        {/* Online bar — no maxUsers number */}
        <div style={{ display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ width:6, height:6, background:barClr, borderRadius:'50%', display:'inline-block', flexShrink:0 }} />
          <span style={{ fontSize:'0.71rem', color:'rgba(255,255,255,.4)', fontWeight:600, flexShrink:0 }}>
            {online} online
          </span>
      ...
      <i className="fi fi-sr-angle-right" style={{ color:'rgba(255,255,255,.18)', fontSize:13, flexShrink:0 }} />
    </div>
  )
}
...
export default function ChatLobby() {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0d1520', display:'flex', flexDirection:'column' }}>
      ...
      {/* Search only — no filter pills */}
      <div style={{ background:'#0f1923', borderBottom:'1px solid #1e2d3d', padding:'9px 14px' }}>
        <div style={{ maxWidth:640, margin:'0 auto', position:'relative' }}>
          <i className="fi fi-sr-search" style={{ position:'absolute', left:11, top:'50%', transform:'translateY(-50%)', color:'#374151', fontSize:12 }} />
          <input
      ...
      <div style={{ flex:1, maxWidth:640, width:'100%', margin:'0 auto' }}>
        ...
        {loading && (
          <div style={{ textAlign:'center', padding:'70px 0' }}>
            ...
            <p style={{ color:'#374151', fontSize:'0.84rem' }}>Loading rooms...</p>
          </div>
        )}
        ...
        {error && (
          <div style={{ margin:'14px', background:'rgba(239,68,68,.1)', border:'1px solid rgba(239,68,68,.25)', borderRadius:9, padding:'12px 14px', color:'#f87171', fontSize:'0.84rem', textAlign:'center' }}>
            ⚠️ {error}
            <button onClick={fetchRooms} style={{ background:'none', border:'none', color:'#60a5fa', fontWeight:700, cursor:'pointer', marginLeft:8 }}>Retry</button>
          </div>
        )}
        ...
        {!loading && !error && (
          <>
            {pinned.length > 0 && (
              <>
                <div style={{ padding:'10px 14px 3px', fontSize:'0.67rem', fontWeight:700, color:'#f59e0b', letterSpacing:'1px', textTransform:'uppercase' }}>
                  📌 Featured
                </div>
                {pinned.map(r => <RoomRow key={r._id} room={r} onClick={joinRoom} />)}
                <div style={{ height:6, background:'rgba(255,255,255,.03)' }} />
              </>
            )}

            <div style={{ padding:'10px 14px 3px', fontSize:'0.67rem', fontWeight:700, color:'#374151', letterSpacing:'1px', textTransform:'uppercase' }}>
        ...
        <div onClick={()=>setPass(null)} style={{ position:'fixed', inset:0, zIndex:1000, background:'rgba(0,0,0,.7)', backdropFilter:'blur(8px)', display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
          <div onClick={e=>e.stopPropagation()} style={{ background:'#111827', border:'1px solid #1f2937', borderRadius:14, padding:'24px 20px', maxWidth:300, width:'100%', boxShadow:'0 20px 60px rgba(0,0,0,.6)' }}>
            <div style={{ textAlign:'center', marginBottom:16 }}>
              <i className="fi fi-sr-lock" style={{ fontSize:30, color:'#6b7280', display:'block', marginBottom:7 }} />
