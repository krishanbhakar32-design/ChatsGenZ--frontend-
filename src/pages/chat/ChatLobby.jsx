 ...
 * ChatLobby.jsx — FIXED
 * Fixes:
 * 1. /icons/ranks/ → /ranks/ (rank icon path)
 * 2. /default_images/avatar/ → /default_avatar/ (getAvatarUrl)
 * 3. Profile dropdown nav → /profile/:username
 * 4. PassModal password check was doing client-side only — now sends via nav with password state
 */
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          <div style={{ minWidth:0 }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:5, marginTop:3 }}>
              {/* FIX: /icons/ranks/ → /ranks/ */}
              <img src={`/ranks/${ri.icon}`} alt="" style={{ width:13, height:13 }} onError={e=>e.target.style.display='none'} />
        ...
        <form onSubmit={submit} style={{ padding:'18px 20px 22px' }}>
          <div style={{ marginBottom:16 }}>
            <label style={{ display:'block', fontSize:'0.8rem', fontWeight:700, color:'#374151', marginBottom:8 }}>
              <i className="fi fi-sr-picture" style={{ marginRight:5, color:'#1a73e8' }}/>Room Image
            </label>
          ...
          <div style={{ marginBottom:14 }}>
            ...
            <textarea style={{...inp, resize:'vertical', minHeight:70, lineHeight:1.5}} placeholder="What's this room about?"
          ...
          <div style={{ marginBottom:14 }}>
            ...
            <input style={inp} type="text" placeholder="Leave empty for no lock" value={form.password}
          ...
          {err && (
            <div style={{ background:'#fef2f2', border:'1px solid #fecaca', borderRadius:8, padding:'9px 13px', fontSize:'0.8rem', color:'#dc2626', marginBottom:14 }}>
              {err}
            </div>
          )}
          ...
          <div style={{ display:'flex', gap:10 }}>
            ...
            <button type="submit" disabled={saving}
              style={{ flex:2, padding:'12px', borderRadius:10, border:'none', background: saving?'#9ca3af':'linear-gradient(135deg,#1a73e8,#1464cc)', color:'#fff', fontWeight:800, cursor:saving?'not-allowed':'pointer', fontSize:'0.875rem', fontFamily:'Outfit,sans-serif' }}>
      ...
      <div onClick={()=>onClick(room)} style={{ display:'flex', alignItems:'center', padding:'12px 14px', gap:12 }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:6 }}>
            ...
            {room.isPinned && <i className="fi fi-sr-thumbtack" style={{ fontSize:11, color:'#f59e0b', marginLeft:2 }}/>}
            <div style={{ flex:1 }}/>
            <span style={{ fontSize:'0.85rem', fontWeight:800, color: online>0?'#22c55e':'#9ca3af' }}>{online}</span>
            <img src="/default_images/rooms/user_count.svg" alt="" style={{ width:16, height:16 }}
...
export default function ChatLobby() {
  ...
  function join(room) {
    ...
    // FIX: pass room password check to ChatRoom via state
    if (room.password) setPassRoom(room)
    else nav(`/chat/${room._id}`)
  }

  // FIX: PassModal now passes password to ChatRoom via location state
